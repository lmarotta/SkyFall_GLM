...
split=1; %flag to split data into test and train sets using LOOCV
class=0; % flag for fall classification (rather than detection only)
fall_like=5; % 5 to set as non-fall, 9 to set as fall
remove_falllike = 0;
remove_activities = 0;
loop2=0;
...
for indCV=1:length(subj)
    ...
    labels_train.value = labels.value(indtrain);
    labels_train.subject = labels.subject(indtrain);
    labels_train.acce = labels.acce(indtrain);
    labels_train.gyro = labels.gyro(indtrain);
    labels_train.baro = labels.baro(indtrain);
