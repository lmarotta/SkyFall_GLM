...
function [Labresults,Homeresults] = GenerateThesisResults
close all
...
pred_all = cell(1,length(subj));
confmat_all=zeros(2,2,length(subj));
misclassified.acce = {};
misclassified.value = [];
misclassified.location = {};
...
if find(cvtype==1)
    ...
    for indCV=1:length(subj)
        ...
        for run = 1:Nruns
            ...
            if model
                ...
                %save misclassified info
                misc = isfall~=pred;
                acceTest = HLabels.acce(indtest,:);
                misclassified.acce = [misclassified.acce; acceTest(misc,:)];
                valTest = HLabels.value(indtest);
                misclassified.value = [misclassified.value; valTest(misc)];
                locTest = HLabels.labels.location(indtest);
                misclassified.location = [misclassified.location; locTest(misc)];
    ...
    % save HH_stats misclassified confmat
...
if any(cvtype == 2)
    ...
    for run = 1:Nruns
        ...
        if model
            ...
            for i = 1:length(unique(X_test(:,1)))
                ...
                %save misclassified info
                misc = isfall~=pred;
                acceTest = ALabels.acce(rowid,:);
                misclassified.acce = [misclassified.acce; acceTest(misc,:)];
                valTest = ALabels.value(rowid);
                misclassified.value = [misclassified.value; valTest(misc)];
                locTest = ALabels.labels.location(rowid);
                misclassified.location = [misclassified.location; locTest(misc)];
    ...
    confmat=sum(confmat_all,3);   
    % save HA_stats misclassified confmat
...
if any(cvtype == 3)
    ...
    for indCV=1:length(subj)
        ...
        for run = 1:Nruns
            ...
            if model
                ...
                %save misclassified info
                misc = isfall~=pred;
                acceTest = ALabels.acce(indtest,:);
                misclassified.acce = [misclassified.acce; acceTest(misc,:)];
                valTest = ALabels.value(indtest);
                misclassified.value = [misclassified.value; valTest(misc)];
                locTest = ALabels.labels.location(indtest);
                misclassified.location = [misclassified.location; locTest(misc)];
            else
                RFModel=TreeBagger(100,F(indtrain,:),L(indtrain));
                [pred,conf]=predict(RFModel,F(indtest,:));
    ...
    confmat=sum(confmat_all,3);
    % save AA_stats misclassified confmat
