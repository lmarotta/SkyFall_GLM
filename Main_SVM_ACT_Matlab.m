...
for indCV=1:length(subj)
    ...
    Model=SVMtrain(F(indtrain,:),L(indtrain));
 
    % Testing the model on Test Data (left out subject)
    confmat = SVMeval(Model,F(indtest,:),L(indtest),1);
    conf_all{indCV}=conf;
...
if amputee_test
    ...
    [pred,conf,confmat] = SVMeval(Model,FAmputee,LAmputee,1);
    %Save model trained on healthy and threshold
    fvar.std=fvar.std(fvar.nzstd);
    fvar.mu=fvar.mu(fvar.nzstd);
