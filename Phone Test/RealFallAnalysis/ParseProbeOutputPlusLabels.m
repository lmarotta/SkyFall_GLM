...
fall_start_position = cell(length(fall_labels),1);
...
for i=1:length(fall_labels)
    ...
    fall_start_position(i) = temp(start_position+2);
...
fall_probab = -ones(length(ind_correct),1);
is_fall= cell(length(ind_correct),1);
...
for ind=1:length(ind_correct)
    ...
    if ~isempty(x)
        fall_probab(ind) = str2double(temp_correct(x+1));
    else
        fall_probab(ind) = 0;
    end
    
    i_f=find(strcmp(temp_correct,'IS_FALL'));
    is_fall_char(ind) = (temp_correct(i_f+1));
...
is_fall = (strcmp(is_fall_char,'true'))';
...
labels.gyro = gyro;
labels.baro = baro;
labels.fall_probab = fall_probab;
labels.is_fall = is_fall;
